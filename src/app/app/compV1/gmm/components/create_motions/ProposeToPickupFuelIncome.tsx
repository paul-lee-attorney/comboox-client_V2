...
import { useGeneralKeeperCreateActionOfGm } from "../../../../../../../generated-v1";
...
export function ProposeToPickupFuelIncome({ refresh }:CreateMotionProps) {
  ...
  const {
    isLoading: proposePickupFuelIncomeLoading,
    write: proposePickupFuelIncome,
  ...
  const handleClick = ()=>{
    if (seqOfVR && executor && amt) {
      proposePickupFuelIncome({
        args: [
          BigInt(seqOfVR), 
          [AddrOfTank],
          [0n],
          [HexParser('0x9273bbb6' + strNumToBigInt(amt, 18).toString(16).padStart(64, '0'))],
  ...
  return (
    ...
    <Paper elevation={3} sx={{ m:1, p:1, color:'divider', border:1 }}  >
      ...
      <Stack direction="row" sx={{ alignItems:'start' }} >
        ...
        <LoadingButton
          disabled={ proposePickupFuelIncomeLoading || hasError(valid)}
          loading={loading}
          loadingPosition="end"
          variant="contained"
          endIcon={<EmojiPeople />}
