...
import { cashier, rate } from '../ft';

export function Refuel({ addrFT, refresh }: ActionOfFuelProps) {
  ...
  useEffect(()=>{
    if (addrFT) {
      cashier().then(
        addrCashier => setEscrow(addrCashier)
      )
    }
  }, [addrFT]);
    ...
    onSuccess(data) {
      ...
      getReceipt(hash).then(
        r => {
          ...
          if (r) {
            let rpt:Receipt = {
              from: r.logs[4].topics[1],
              to: r.logs[4].topics[2],
              amt: BigInt(r.logs[4].data).toString(),
              };
            setReceipt(rpt);
            setOpen(true);
            setLoading(false);
