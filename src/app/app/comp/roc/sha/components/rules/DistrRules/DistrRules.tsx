...
import { SetDistrRule, } from "./SetDistrRule";
...
export function DistrRules({sha, initSeqList, isFinalized, time, refresh}: GroupRulesSettingProps) {

  const { setErrMsg } = useComBooxContext();

  const mandatoryRules: number[] = isFinalized ? [] : [1280];
              ...
              <Grid container spacing={0.5} >
                ...
                {cp.map(v=> (
                  <Grid key={ v } item xs={3} >
                    <SetDistrRule  sha={ sha } seq={ v } isFinalized={ isFinalized } time={time} refresh={ refresh } />
                  </Grid>
                ))}
