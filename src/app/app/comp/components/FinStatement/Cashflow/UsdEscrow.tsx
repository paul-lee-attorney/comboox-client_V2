...
export function UsdEscrow({setRecords}:CashflowRecordsProps) {
  ...
  useEffect(()=>{
    ...
    const getUsdEscrow = async () => {
      ...
      let abiStr = 'event ForwardUsd(address indexed from, address indexed to, uint256 indexed amt, bytes32 remark)';
      ...
      let forwardUsdLogs: TypeOfCashierLog[] = [];
      
      if (rawLogs && rawLogs.length > 0) {
        forwardUsdLogs = rawLogs.map(log => decodeArbiscanLog(log, abiStr) as TypeOfCashierLog);
      }
      ...
      let releaseUsdLogs: TypeOfCashierLog[] = [];
      
      if (rawLogs && rawLogs.length > 0) {
        releaseUsdLogs = rawLogs.map(log => decodeArbiscanLog(log, abiStr) as TypeOfCashierLog);
      }
      ...
      let custodyUsdLogs: TypeOfCashierLog[] = [];
      
      if (rawLogs && rawLogs.length > 0) {
        custodyUsdLogs = rawLogs.map(log => decodeArbiscanLog(log, abiStr) as TypeOfCashierLog);
      }
      ...
      rawLogs = await getNewLogs(gk, 'Cashier', cashier, 'DistrProfits', fromBlkNum);

      abiStr = 'event DistrProfits(uint256 indexed amt, uint256 indexed seqOfDR, uint256 indexed seqOfDistr)';

      type TypeOfDistrProfitsLog = ArbiscanLog & {
        eventName: string, 
        args: {
          amt: bigint,
          seqOfDR: bigint,
      ...
      let distributeUsdLogs: TypeOfDistrProfitsLog[] = [];
      
      if (rawLogs && rawLogs.length > 0) {
        distributeUsdLogs = rawLogs.map(log => decodeArbiscanLog(log, abiStr) as TypeOfDistrProfitsLog);
      }
      ...
      rawLogs = await getNewLogs(gk, 'Cashier', cashier, 'DistrIncome', fromBlkNum);

      abiStr = 'event DistrIncome(uint256 indexed amt, uint256 indexed seqOfDR, uint256 fundManager, uint256 indexed seqOfDistr)';

      type TypeOfDistrIncomeLog = ArbiscanLog & {
        eventName: string, 
        args: {
          amt: bigint,
          seqOfDR: bigint,
      ...
      let distrIncomeLogs: TypeOfDistrIncomeLog[] = [];
      
      if (rawLogs && rawLogs.length > 0) {
        distrIncomeLogs = rawLogs.map(log => decodeArbiscanLog(log, abiStr) as TypeOfDistrIncomeLog);
      }

      console.log('distrIncomeLogs: ', distrIncomeLogs);

      len = distrIncomeLogs.length;
      cnt = 0;

      while (cnt < len) {

        let log = distrIncomeLogs[cnt];
      ...
      abiStr = 'event PickupUsd(address indexed msgSender, uint256 indexed caller, uint256 indexed value)';
      ...
      let pickupUsdLogs: TypeOfPickupUsdLog[] = [];
      
      if (rawLogs && rawLogs.length > 0) {
        pickupUsdLogs = rawLogs.map(log => decodeArbiscanLog(log, abiStr) as TypeOfPickupUsdLog);
      }
