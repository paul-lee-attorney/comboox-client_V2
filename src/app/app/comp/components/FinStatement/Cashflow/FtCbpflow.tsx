...
import { AddrOfRegCenter, AddrZero, Bytes32Zero, } from "../../../../common";
...
export function FtCbpflow({setRecords}:CashflowRecordsProps ) {
  ...
  useEffect(()=>{
    ...
    const getFtCashflow = async ()=>{
      ...
      console.log('toBlkNum of FtCbpflow: ', toBlkNum);
      ...
      let abiStr = 'event Transfer(address indexed from, address indexed to, uint256 value)';
      ...
      rawLogs = await getNewLogs(gk, 'FuelTank', HexParser(fuelTanks.FT0) , 'WithdrawFuel', fromBlkNum);

      abiStr = 'event WithdrawFuel(address indexed owner, uint256 indexed amt)';
      ...
      rawLogs = await getNewLogs(gk, 'FuelTank', HexParser(fuelTanks.FT0) , 'Refuel', fromBlkNum);

      abiStr = 'event Refuel(address indexed buyer, uint256 indexed amtOfEth, uint256 indexed amtOfCbp)';
      ...
      if (arr.length > 0) {
        ...
        await setFinData(gk, 'ftCbpflow', arr, toBlkNum);
