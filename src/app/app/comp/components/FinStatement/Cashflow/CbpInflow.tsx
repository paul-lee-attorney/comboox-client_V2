    ...
    const getCbpInflow = async () => {
      ...
      const updateRoyaltyInfo = async (item:Cashflow) => {
        ...
        if (tran.to?.toLowerCase() == gk.toLowerCase()) {
          ...
          const selector = tran.input.substring(0, 10).toLowerCase();
          item.api = selectorToFunctionName[selector as keyof typeof selectorToFunctionName] ?? '';
