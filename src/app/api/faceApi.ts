...
export async function checkFaceAct (
  video: HTMLVideoElement, 
  checkAct: (video: HTMLVideoElement, prePosition: HeadPosition | null) => Promise<boolean | null>
): Promise<boolean> {
  ...
  let intervalId = setInterval(async () => {
    const flag = await checkAct(video, previousHeadPosition);
...
function euclideanDistance (point1: faceapi.Point, point2: faceapi.Point) {
  return Math.hypot(point2.x - point1.x, point2.y - point1.y);
};

export async function checkEyeBlink (video: HTMLVideoElement, prePosition: HeadPosition | null): Promise<boolean | null> {
...
export async function checkHeadTurn (video: HTMLVideoElement, prePosition: HeadPosition | null): Promise<boolean | null> {
...
export async function checkHeadNode (video: HTMLVideoElement, prePosition: HeadPosition | null): Promise<boolean | null> {
...
export async function checkMouthOpen(video: HTMLVideoElement, prePosition: HeadPosition | null): Promise<boolean | null> {
